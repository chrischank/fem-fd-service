...
BUILD_TAG := env_var_or_default("BUILD_TAG", "latest")
DOCKERIZE_HOST := `echo "${GOOSE_DBSTRING:-}" | cut -d "@" -f 2 | cut -d ":" -f 1`
DOCKERIZE_URL := if DOCKERIZE_HOST != "" { "tcp://" + DOCKERIZE_HOST + ":5432" } else { "tcp://localhost:5432" }
